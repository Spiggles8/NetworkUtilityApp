...
namespace NetworkUtilityApp.Helpers
{
  ...
  internal static class LlmnrResolver
  {
    ...
    // ip: Target IPv4 address (string form).
    // timeoutMs: Receive timeout in milliseconds.
    // localBindIp: Optional local IPv4 to bind the UDP socket to (useful when multiple interfaces are present).
