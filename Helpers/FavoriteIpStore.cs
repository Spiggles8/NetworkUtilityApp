...
{
  /// <summary>
  /// Persisted store for up to four Favorite IP presets.
  /// Loads/saves `favorites.json` (preferring app-local, falling back to AppData),
  /// provides get/save helpers with change notification, and seeds sensible defaults.
  /// </summary>
  ...
  public static class FavoriteIpStore
  {
    ...
    private static FavoriteIpEntry[] BuildDefaultFavorites()
    {
      ...
      return
      [
          new FavoriteIpEntry { Ip = "192.168.1.8", Subnet = "255.255.255.0", Gateway = "" },
                new FavoriteIpEntry { Ip = "192.168.0.8", Subnet = "255.255.255.0", Gateway = "" },
                new FavoriteIpEntry { Ip = "10.0.0.8", Subnet = "255.255.255.0", Gateway = "" },
                new FavoriteIpEntry { Ip = "172.22.0.8", Subnet = "255.255.255.0", Gateway = "" },
            ];
    }
  }
}