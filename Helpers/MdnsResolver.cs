...
namespace NetworkUtilityApp.Helpers
{
  ...
  internal static class MdnsResolver
  {
    ...
    // ip: Target IPv4/IPv6 address as string.
    // timeoutMs: Overall receive timeout for mDNS replies.
    // localBindIp: Optional local IP to bind the UDP socket to.
    public static string TryGetHostname(string ip, int timeoutMs = 1500, string? localBindIp = null)
    {
      try
      {
        ...
        if (!string.IsNullOrWhiteSpace(localBindIp) && IPAddress.TryParse(localBindIp, out var bindAddr))
        {
          ...
          try { udp.Client.SetSocketOption(SocketOptionLevel.IP, SocketOptionName.MulticastInterface, bindAddr.GetAddressBytes()); }
          catch { }
        }
    ...
    // A/AAAA RR given the start of the record by walking backwards
    // to a plausible NAME boundary and then reading the name.
